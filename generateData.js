...
const generateData = () => {
  ...
  // Averages: Alter=54.56, Dienstjahre=26.38, Brutto=5819.31, Skill=0.4
  for (let i = 0; i < 1250; i++) {
    ...
    const kosten = (parseFloat(brutto) * dienstjahre * 1.5).toFixed(2); // roughly
    data.push(`${id++},Stahl_Produktion_GmbH,${alter},${dienstjahre},${brutto},${skill},${montan},${status},${kosten}`);
  }

  // Logistik_Sued_GmbH: 750 employees
  // Averages: Alter=41.76, Dienstjahre=13.9, Brutto=4144.02, Skill=0.59
  for (let i = 0; i < 750; i++) {
  ...
  // Averages: Alter=34.72, Dienstjahre=6.45, Brutto=6487.74, Skill=0.85
  for (let i = 0; i < 500; i++) {
